<?php
/**
 * Custom participant list to diplay name and role
 *
 * @author Thomas Blein (Aqua Paris Plongee) <webmaster@aquaparisplongee.fr>
 * @license AGPL-3.0
 */
class CRM_Event_Page_ParticipantListing_NameAndRole extends CRM_Core_Page {
...
LIMIT    $offset, $rowCount";
    ...
    $roleLookup = CRM_Event_PseudoConstant::participantRole();
    while ($object->fetch()) {
      $role = CRM_Utils_Array::value($object->role_id, $roleLookup);
      if ($role) {
        $role = ts($role);
      }
      $row = array(
